...
class SncmdbDataSource(BaseDataSource):
    ...
    def _check_cache(self, sn_table):
        print(self._sync_cursor)
        if sn_table in self._sync_cursor[CURSOR_TABLE_KEY]:
            print(f'checking cursor:{self._sync_cursor} {sn_table}')
            return self._sync_cursor[CURSOR_TABLE_KEY][sn_table]
        return None
